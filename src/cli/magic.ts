...
import {UpdateSelf} from "./updater";
...
${PNAME} --update --version=v0.0.4 ~ Update Deviation to v0.0.4

${colors.cyan('Flags')} (all flags begin with '--')
help            ~ Show this message
update          ~ Update Deviation to the selected version
...
`);
    ...
    if (args.is("update")) {
        UpdateSelf(args.option("version") || "latest");
        return false;
    }
